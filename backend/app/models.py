from sqlalchemy import Column, String, Integer, Float, DateTime, Boolean, ForeignKey, JSON, Text, Enum
...
class WorkoutPlan(Base):
    ...
    focus_areas = Column(JSON)  # e.g., ["chest", "back"]
    generated_by = Column(String, default="ai")  # ai or coach
...
class Exercise(Base):
    ...
    equipment = Column(JSON)  # ["dumbbell", "barbell"]
...
class DietPlan(Base):
    ...
    scientific_basis = Column(Text, nullable=True)  # Explanation of research backing
    evidence_level = Column(String, default="moderate")  # high, moderate, preliminary
    research_citations = Column(JSON, nullable=True)  # Array of PubMed citations
    research_verified = Column(Boolean, default=True)  # Was plan created with PubMed research?
    # Persist supplement recommendations (array of objects with dosing, citations, safety notes)
...
class Meal(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    diet_plan_id = Column(String, ForeignKey("diet_plans.id"))
    name = Column(String)
    ...
    nutritional_benefits = Column(Text, nullable=True)  # Why these foods are recommended
    research_backed = Column(Boolean, default=True)  # Is this meal based on research?
    created_at = Column(DateTime, default=datetime.utcnow)
...
class ProgressEntry(Base):
    ...
    measurements = Column(JSON, nullable=True)  # chest, waist, hips, etc.
...
class Message(Base):
    ...
    user_id = Column(String, ForeignKey("users.id"))
    coach_id = Column(String, nullable=True)
    sender_type = Column(String)  # user, coach, ai
    content = Column(Text)
    message_type = Column(String, default="text")  # text, form_check, nutrition, etc.
...
class Reminder(Base):
    ...
    title = Column(String)
    message = Column(Text)
    remind_at = Column(DateTime, nullable=False)  # next scheduled reminder datetime
    repeat = Column(String, nullable=True)  # ISO interval or cron-like string (optional)
    channel = Column(String, default="in-app")  # in-app, push, email
...
class Subscription(Base):
    ...
    plan_type = Column(String)  # free, pro, elite
