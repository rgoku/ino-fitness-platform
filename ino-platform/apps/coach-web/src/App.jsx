...
import {
  ...
  Sparkles, LayoutDashboard, PanelLeftClose, PanelLeft, LogOut
} from "lucide-react";
import {
...
// MAIN APP
// ═══════════════════════════════════════════════════
export default function INOCoachCommandCenter({ user, onLogout } = {}) {
  ...
  const userInitials = userName.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2) || "SM";
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <div style={{
            ...
            <Avatar ini={userInitials} c={t.accent} sz={32} />
            {!sidebarCollapsed && (
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: t.text }}>{userName}</div>
                <div style={{ fontSize: 10, color: t.textDim }}>Pro Plan · {totalClients} clients</div>
              </div>
            )}
          </div>
          {onLogout && (
            <button onClick={onLogout} style={{
              ...
              gap: 10, padding: sidebarCollapsed ? "10px" : "10px 12px",
              background: "none", border: "none", borderRadius: 10,
              cursor: "pointer", color: t.danger, fontSize: 12, fontWeight: 500, fontFamily: "inherit",
